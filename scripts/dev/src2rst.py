...
"""Generate ReST source for qutebrowser based on docstrings."""
...
class UsageFormatter(argparse.HelpFormatter):
    ...
    def _metavar_formatter(self, action, default_metavar):
        """Override _metavar_formatter to add ReST markup to metavars.
    ...
    def _format_actions_usage(self, actions, groups):
        """Override _format_actions_usage to add ReST markup to flags.

        Because argparse.py's _format_actions_usage is very complex, we first
        monkey-patch the option strings to include the ReST markup, then
...
def _get_command_quickref(cmds):
    ...
    out = []
    out.append('.. csv-table:: Commands')
    out.append('   :header: "Command", "Description"')
    out.append('   :widths: 25, 75')
    out.append('   ')
    for name, cmd in cmds:
        desc = inspect.getdoc(cmd.handler).splitlines()[0]
        out.append('   :ref:`{} <cmd-{}>`, "{}"'.format(name, _label(name), desc))
    return '\n'.join(out)
...
def _get_setting_quickref():
    ...
    out = []
    out.append('.. csv-table:: Settings')
    out.append('   :header: "Setting", "Description"')
    out.append('   :widths: 25, 75')
    out.append('   ')
    for opt in sorted(configdata.DATA.values()):
        desc = opt.description.splitlines()[0]
        out.append('   :ref:`{} <setting-{}>`, "{}"'
...
def _get_command_doc(name, cmd):
    ...
    output = ['', '.. _cmd-{}:'.format(_label(name)), '']
...
def _generate_setting_option(f, opt):
    ...
    f.write("\n")
    f.write('.. _setting-{}:'.format(_label(opt.name)) + "\n\n")
...
def regenerate_manpage(filename):
    ...
    for group in parser._action_groups:
        ...
        groupdata.append('^' * len(group.title))
...
def main():
    ...
    regenerate_manpage('doc/qutebrowser.1.rst')
    print("Generating settings help...")
    generate_settings('doc/settings.rst')
    print("Generating command help...")
    generate_commands('doc/commands.rst')
